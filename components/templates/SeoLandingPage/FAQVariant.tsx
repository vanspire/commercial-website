        ...
        <div className="space-y-4">
          {data.items.map((faq, index) => {
            ...
            return (
              <div 
                key={index} 
                className="bg-white border border-brand-border overflow-hidden transition-all duration-300"
              >
                <button
                  className="w-full flex items-center justify-between p-6 md:p-8 text-left focus:outline-none"
                  onClick={() => setOpenIndex(isOpen ? null : index)}
                >
                  <h3 className="text-lg md:text-xl font-medium text-brand-black pr-8">
                    {faq.question}
                  </h3>
                  <div className="shrink-0 w-8 h-8 flex items-center justify-center border border-brand-black/20 rounded-full">
